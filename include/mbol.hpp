...
string convert (set<Element, ElementCompare> x) {
  Element b = x;
  return b.print();
}
...
MBOLEnv g_env;
class MBOLExpr : public IloExpr {
  public:
  MBOLExpr (const double& a) : IloExpr(g_env) {
    operator+=(a);
  }
  MBOLExpr (const IloNumExprArg& a) : IloExpr(g_env) {
    operator+=(a);
  }
  MBOLExpr() : IloExpr (g_env) {
  }
};
...
enum EXPR_TYPE {EXPR, INT, NUM};
  ...
  operator string() const {
    return "just making stuff compile";
  }
  void simplify() {
